...
static void Esp32WeatherClear(WeatherInfo *weather)
{
	...
	weather->todayCodeDay = -1;
	weather->todayCodeNight = -1;
}


static uint8_t Esp32IsValidUtf8(const char *text)
{
...
static void Esp32LogInvalidUtf8(const char *key, const char *value)
{
	char hexbuf[64];
	uint16_t i = 0U;
	uint16_t used = 0U;
	...
	hexbuf[0] = '\0';
	while ((bytes[i] != 0U) && (i < maxLen) && (used + 3U < (uint16_t)sizeof(hexbuf)))
	{
		n = snprintf(&hexbuf[used], (size_t)(sizeof(hexbuf) - used), "%02X ", bytes[i]);
		if (n <= 0)
		{
			break;
		}
	...
	printf("[ESP32][ENC] invalid UTF-8 key=%s raw='%s' hex=%s\r\n", key, value, hexbuf);
}
...
static void Esp32ParseWeatherItem(WeatherInfo *weather, const char *key, const char *value)
{
	...
	{
		weather->textUtf8Valid = Esp32IsValidUtf8(value);
		if (weather->textUtf8Valid == 0U)
		{
			Esp32LogInvalidUtf8(key, value);
		}
		Esp32SafeCopy(weather->text, (uint16_t)sizeof(weather->text), value);
	}
	else if ((strcmp(key, "icon") == 0) ||
			 (strcmp(key, "code") == 0) ||
			 (strcmp(key, "weatherCode") == 0))
	{
		weather->weatherCode = atoi(value);
		weather->hasWeatherCode = 1U;
	}
	...
	else if (strcmp(key, "todayTempMin") == 0)
	{
		weather->todayTempMin = atoi(value);
			 ...
			 (strcmp(key, "textDay") == 0) ||
			 (strcmp(key, "dayText") == 0))
	{
		weather->todayTextDayUtf8Valid = Esp32IsValidUtf8(value);
	...
	else if ((strcmp(key, "todayCodeDay") == 0) ||
			 (strcmp(key, "codeDay") == 0) ||
			 (strcmp(key, "iconDay") == 0) ||
			 (strcmp(key, "dayCode") == 0) ||
			 (strcmp(key, "dayIcon") == 0))
	{
		weather->todayCodeDay = atoi(value);
		weather->hasTodayCodeDay = 1U;
	}
	else if ((strcmp(key, "todayTextNight") == 0) ||
			 (strcmp(key, "textNight") == 0) ||
			 (strcmp(key, "nightText") == 0))
	{
		weather->todayTextNightUtf8Valid = Esp32IsValidUtf8(value);
	...
	else if ((strcmp(key, "todayCodeNight") == 0) ||
			 (strcmp(key, "codeNight") == 0) ||
			 (strcmp(key, "iconNight") == 0) ||
			 (strcmp(key, "nightCode") == 0) ||
			 (strcmp(key, "nightIcon") == 0))
	{
		weather->todayCodeNight = atoi(value);
		weather->hasTodayCodeNight = 1U;
	}
}
