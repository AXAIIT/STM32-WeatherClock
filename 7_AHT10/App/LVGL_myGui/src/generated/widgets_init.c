...
static int screen_1_date_year_value = 2023;
static int screen_1_date_month_value = 7;
static int screen_1_date_day_value = 31;
static bool screen_1_date_synced = false;

static int screen_1_calc_weekday_monday0(int year, int month, int day)
{
    int y;
    int m;
    int k;
...
static void screen_1_update_weekday_span(void)
{
    int weekday_index;

    if(!lv_obj_is_valid(guider_ui.screen_1_spangroup_12)) {
        return;
    }

    if(guider_ui.screen_1_spangroup_12_span == NULL) {
        return;
    }

    weekday_index = screen_1_calc_weekday_monday0(screen_1_date_year_value,
                                                  screen_1_date_month_value,
                                                  screen_1_date_day_value);
    custom_set_weekday_text(&guider_ui, weekday_index);
}

static void screen_1_sync_date_from_label(void)
{
    int year;
    int month;
    int day;
    ...
    if(screen_1_date_synced) {
        return;
    }

    if(!lv_obj_is_valid(guider_ui.screen_1_datetext_1)) {
        return;
    }

    date_text = lv_label_get_text(guider_ui.screen_1_datetext_1);
    if(date_text != NULL && sscanf(date_text, "%d/%d/%d", &year, &month, &day) == 3) {
        ...
        screen_1_date_year_value = year;
        screen_1_date_month_value = month;
        screen_1_date_day_value = day;
        screen_1_update_weekday_span();
    }

    screen_1_date_synced = true;
}

static void screen_1_set_date_label_text(void)
{
    if(lv_obj_is_valid(guider_ui.screen_1_datetext_1)) {
        lv_label_set_text_fmt(guider_ui.screen_1_datetext_1,
                              "%d/%02d/%02d",
                              screen_1_date_year_value,
                              screen_1_date_month_value,
                              screen_1_date_day_value);
    }
...
static bool screen_1_is_midnight_after_tick(void)
{
    return (screen_1_digital_clock_1_hour_value == 12) &&
           (screen_1_digital_clock_1_min_value == 0) &&
           (screen_1_digital_clock_1_sec_value == 0) &&
...
void screen_1_digital_clock_1_timer(lv_timer_t *timer)
{
    ...
    if(screen_1_is_midnight_after_tick()) {
        date_count(&screen_1_date_year_value, &screen_1_date_month_value, &screen_1_date_day_value);
        screen_1_set_date_label_text();
    }
...
static lv_calendar_date_t screen_1_calendar_highlighted_days[1];
...
{
    if (screen_1_datetext_1_calendar == NULL) {
        int year = screen_1_date_year_value;
        int month = screen_1_date_month_value;
        int day = screen_1_date_day_value;
        ...
        if(s != NULL) {
            ...
            if(sscanf(s, "%d/%d/%d", &parsed_year, &parsed_month, &parsed_day) == 3) {
                year = parsed_year;
                month = parsed_month;
                day = parsed_day;
            }
        ...
        lv_calendar_set_showed_date(screen_1_datetext_1_calendar, year, month);
        screen_1_calendar_highlighted_days[0].year = year;
        screen_1_calendar_highlighted_days[0].month = month;
        screen_1_calendar_highlighted_days[0].day = day;
        lv_calendar_set_highlighted_dates(screen_1_datetext_1_calendar, screen_1_calendar_highlighted_days, 1);
...
void screen_1_datetext_1_calendar_event_handler(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        screen_1_date_year_value = date.year;
        screen_1_date_month_value = date.month;
        screen_1_date_day_value = date.day;
        screen_1_date_synced = true;
        screen_1_set_date_label_text();
